...
const TorBridge = _reactNative.NativeModules.TorBridge || {
  startDaemon: () => Promise.reject(new Error('TorBridge not available')),
  stopDaemon: () => Promise.reject(new Error('TorBridge not available')),
  getDaemonStatus: () => Promise.resolve('NOTINIT'),
  request: () => Promise.reject(new Error('TorBridge not available')),
  startTcpConn: () => Promise.reject(new Error('TorBridge not available')),
  sendTcpConnMsg: () => Promise.reject(new Error('TorBridge not available')),
  stopTcpConn: () => Promise.reject(new Error('TorBridge not available')),
};
/**
...
var _default = function _default() {
  ...
  const startIfNotStarted = () => {
    if (!bootstrapPromise) {
      bootstrapPromise = _reactNative.NativeModules.TorBridge.startDaemon(bootstrapTimeoutMs, clientTimeoutSeconds);
    }
